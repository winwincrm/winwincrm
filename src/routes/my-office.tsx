...
function MyOfficePage() {
  return (
    <ProtectedRoute roles={["manager", "superiormanager", "admin"]}>
      <MyOfficeContent />
    </ProtectedRoute>
  );
}
...
function MyOfficeContent() {
  ...
  const load = async () => {
    ...
    const [{ data: officeProfiles }, { data: agentRoles }, { count: leads }] = await Promise.all([
      supabase.from("profiles").select("user_id").eq("office_id", profile.office_id),
    ...
    const agentIds = new Set((agentRoles ?? []).map((row) => row.user_id));
    const agents = (officeProfiles ?? []).filter((row) => agentIds.has(row.user_id)).length;
    setStats({ agents, leads: leads ?? 0 });
  };
  ...
  return (
    <div className="space-y-5 max-w-3xl">
      <h1 className="text-2xl font-semibold tracking-tight">
        {t("my_office.title", { defaultValue: "My Office" })}
      </h1>
      ...
      <Card>
        <CardHeader>
          <CardTitle className="text-base">
            {t("my_office.details", { defaultValue: "Office details" })}
          </CardTitle>
        </CardHeader>
