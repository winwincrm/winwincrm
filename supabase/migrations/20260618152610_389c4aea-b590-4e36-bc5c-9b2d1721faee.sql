...
BEGIN
  ...
  SELECT
    ...
    l.lead_kind::text,
    l.is_in_house,
    l.hide_in_house_from_agents,
    l.origin_agent_id,
    l.origin_agent_name
  FROM public.leads l
  WHERE
    (
      (v_is_admin AND (p_office IS NULL OR l.office_id = p_office))
    ...
    AND (p_group IS NULL OR p_group = 'all' OR (
         (p_group = 'new' AND l.status::text IN ('new'))
      OR (p_group = 'in_progress' AND l.status::text IN ('no_answer_1','no_answer_2','no_answer_3','wrong_number','try_again','not_interested'))
      OR (p_group = 'callback' AND l.status::text IN ('callback'))
      OR (p_group = 'appointment' AND l.status::text IN ('appointment'))
      OR (p_group = 'converted' AND l.status::text IN ('converted'))
      OR (p_group = 'bad' AND l.status::text IN ('bad_quality','rejected'))
    ))
  ORDER BY
    CASE WHEN v_is_agent THEN l.assigned_at ELSE NULL END DESC NULLS LAST,
    l.created_at DESC
  OFFSET v_offset
  LIMIT v_limit;
END;
...
GRANT EXECUTE ON FUNCTION public.leads_page(uuid, uuid, boolean, text, text, text, text, date, date, text, text, text, integer, integer) TO authenticated;
GRANT EXECUTE ON FUNCTION public.leads_page(uuid, uuid, boolean, text, text, text, text, date, date, text, text, text, integer, integer) TO service_role;