...
-- Drop old single-value signatures
DROP FUNCTION IF EXISTS public.leads_page(uuid, uuid, boolean, text, text, text, text, date, date, text, text, text, integer, integer);
DROP FUNCTION IF EXISTS public.leads_group_counts(uuid, uuid, boolean, text, text, text, text, date, date, text);

-- New multi-value leads_group_counts
...
BEGIN
  ...
  WITH scoped AS (
    ...
    FROM public.leads l
    WHERE (v_allow_all OR v_scope_office IS NOT NULL OR (p_office IS NOT NULL AND array_length(p_office,1) > 0))
...
RETURNS TABLE(
  id uuid, full_name text, first_name text, last_name text, phone text, email text,
  status lead_status, office_id uuid, assigned_user_id uuid, source text, platform text,
  amount numeric, percentage numeric, timeframe text, payload jsonb,
  last_contacted_at timestamptz, created_at timestamptz, updated_at timestamptz,
  assigned_at timestamptz, lead_kind text, is_in_house boolean,
  hide_in_house_from_agents boolean, origin_agent_id uuid, origin_agent_name text
)
LANGUAGE plpgsql
STABLE SECURITY DEFINER
SET search_path TO 'public'
...
BEGIN
  ...
  IF v_is_admin THEN
    RETURN QUERY
    SELECT l.id, l.full_name, l.first_name, l.last_name, l.phone, l.email, l.status,
    ...
    FROM public.leads l
    WHERE (p_office IS NULL OR array_length(p_office,1) IS NULL OR l.office_id = ANY(p_office))
      AND (
        (p_agent IS NULL OR array_length(p_agent,1) IS NULL) AND (p_unassigned IS NULL)
        OR (p_unassigned = true AND l.assigned_user_id IS NULL)
        OR (p_agent IS NOT NULL AND array_length(p_agent,1) > 0 AND l.assigned_user_id = ANY(p_agent))
      ...
      AND (p_status IS NULL OR array_length(p_status,1) IS NULL OR l.status::text = ANY(p_status))
...
REVOKE ALL ON FUNCTION public.leads_group_counts(uuid[], uuid[], boolean, text[], text[], text[], text, date, date, text) FROM PUBLIC, anon;
REVOKE ALL ON FUNCTION public.leads_page(uuid[], uuid[], boolean, text[], text[], text[], text, date, date, text, text, text[], integer, integer) FROM PUBLIC, anon;
